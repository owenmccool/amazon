...
df.snake.sur <- read.csv("Survey_Snakes.csv") #reading in csv file
...
df.snake.sur <- df.snake.sur[ ,-24] #removing column 24
...
# make a new column, sampling_hrs, and use difftime to populate

# is only relevant to the standard night survey sampling effort

# first, join date and start time, and then date and end time
...
sp.abun <- df_snakes.sur %>%
  group_by(Station, Trail_Walked, Date, Species, Diet_Type) %>%
  tally()
...
sp.abun$st.tr.date <- paste(sp.abun$Station, sp.abun$Trail_Walked, sp.abun$Date, sp.abun$Diet_Type, sep = "//")
