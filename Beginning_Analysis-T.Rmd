...
snake.sur$Date_End_Time <- paste(snake.sur$End_Date, snake.sur$Survey_End_Time, sep = " ")

snake.sur$Date_Start_Time <- mdy_hm(snake.sur$Date_Start_Time) #changing start time columns to fix negative survey time length issues
...
pooled.detection.plot <- snk.sur.mat %>%
  ggplot(aes(x= Station, y= total.hr, color = Station)) + #aesthetics, telling it what you want x and y axis to be
