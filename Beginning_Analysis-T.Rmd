...
snake.sur <- read.csv("Snake_Surveys.csv", na.strings = c("", "NA")) #reading in csv file
...
colnames(df.snake.non.sur) <- df.snake.non.sur[1, ] #takes row 1 and makes them column names 
...
occ <- snake.master %>% #need to get snake.master updated to new csv combo
  group_by(Station, Zone, Species, Date) %>%
  tally()
...
occ$st.zn.date <- paste(occ$Station, occ$Zone, 
                               occ$Species, occ$Date, sep = "//")
...
snake.occ.mat <- occ %>% 
  pivot_wider(names_from = Species, 
              values_from = n, 
              values_fill = 0) %>%
  column_to_rownames(var = "st.zn.date")

# cool, now, we need to pull the info out of the row names and make that info columns again. Note, there is probably a better way to do this but here we are. 

snake.occ.mat$st.zn.date <- rownames(snake.occ.mat)

# now you have a column with that whole mess of info
# let's separate the info back out into 3 separate columns
snake.occ.mat <- snake.occ.mat %>% 
  separate(st.zn.date, c('Station', 'Zone', 'Species', 'Date'), sep = "//")
