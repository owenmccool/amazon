...
# get abundance next
snk.sur.mat$total_found <- rowSums(snk.sur.mat[1:17])
...
snk.sur.mat$simpson_diversity <- diversity(snk.sur.mat[1:17], index = "simpson")

# remove temp df
rm(temp)

# and lets get species richness
snk.sur.mat$richness <- specnumber(snk.sur.mat[1:17])
...
predictors <- snake.occ.mat[, c(1,2)] #separating non-snake stuff (station)
...
bc.dis.mat <- vegdist(community, method = "bray")

pnova.bc.sp <- adonis2(bc.dis.mat ~ predictors$Station, method = "bray")
...
horn.dis.mat <- vegdist(community, method = "horn")

pnova.horn.sp <- adonis2(horn.dis.mat ~ predictors$Station)

# look at results
print(pnova.horn.sp)
```
```{r nmds visualization with horn}
nmds.horn <- metaMDS(community, distance = "horn", autotransform = TRUE, maxtry = 200)
