...
occ <- snake.sur %>% #need to get snake.master updated to new csv combo
...
occ$st.diet <- paste(occ$Station, occ$Diet_Type, sep = "//")

# now, let's use dplyr again to transpose (flip) your sp.abun dataframe so that now, you have the species as column names and the number of times they were found at each trail and date will be the data reported (expect lots of 0s and 1s)

snake.occ.mat <- occ %>% 
  pivot_wider(names_from = Diet_Type, #makes the column into the rownames
              values_from = n, 
              values_fill = 0) %>% #will fill spaces with no snakes as 0
  column_to_rownames(var = "st.diet") #will make this new combo the unique identifier as rownames

#snake.occ.mat <- snake.occ.mat[, -23] #removing NA column (23)


```

``` {r Pull Out Predictors and Community Data}

community <- snake.occ.mat[, c(2:3)] #made matrix from all data columns with n snake data


predictors <- as.data.frame(snake.occ.mat[, 1]) #separating non-snake stuff (station)
colnames(predictors)[1] <- "Station"

Jaccard.dis.mat <- vegdist(community, method = "jaccard")


pnova.jac.sp <- adonis2(Jaccard.dis.mat ~ predictors$Station, method = "jaccard")
