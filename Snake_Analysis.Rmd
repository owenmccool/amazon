...
snk.sur.mat <- temp %>% 
  pivot_wider(names_from = Species, 
              values_from = n, 
              values_fill = 0) %>%
  column_to_rownames(var = "st.date.effort")

rm(temp)

jaccard_dissimilarity<- as.matrix(vegdist(snk.sur.mat[, 4:26], method = "jaccard", binary = TRUE)) #dissimilarity matrix, 0 = complete similarity and 1 is complete dissimlarity?

horn.mor <- as.matrix(vegdist(snk.sur.mat[, 4:26], method = "horn"))
