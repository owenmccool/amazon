...
snakes.sur <- snakes.sur %>%
  mutate(Species = ifelse(Species == "Helicops_angulatus_", 
                          "Helicops_angulatus", Species)) # TEMPORARY FIX FOR HELICOPS ANGULATRUS
...
diss.mat.jacc <- vegdist(snk.sur.mat[, 4:20], method = "jaccard")
...
jacc.nmds.pts <- jacc.nmds.pts %>%
  rownames_to_column(var = "DeploymentID")

jacc.nmds.pts <- left_join(jacc.nmds.pts, ct.temp)

jacc.p <- ggplot(jacc.nmds.pts, aes(x = MDS1, y = MDS2, fill = Station)) +
