...
#### ANY DATA FOR TEMP/HUM IS BAD ON AND AFTER JULY 11 2025
...
```{r small mammal data model}

mean <- mean(small.mamms.temp$smammals.hr)
mean

hist(small.mamms.temp$smammals.hr, breaks = 15)
hist(small.mamms.temp$Number_of_Small_Mammals_Spotted, breaks = 10)

ggplot(small.mamms.temp, aes(x = Number_of_Small_Mammals_Spotted)) +
  geom_histogram(bins = 10, color = "black", fill = "cornflowerblue", alpha = 0.8) +
  theme_bw() +
  xlab("Number of Small Mammals Spotted") +
  ylab("Frequency") +
  ggtitle("Distribution of Small Mammal Sightings") +
  theme(
    axis.text = element_text(color = "black", size = 10),
    plot.title = element_text(size = 20, family = "Tahoma"),
    text = element_text(family = "Tahoma", size = 14)
  )


glm <- glmmTMB(Number_of_Small_Mammals_Spotted ~ Station + offset(log(Effort)), data = small.mamms.temp, family = poisson) # offset: term that you suspect will have a positive effect on response variable: which is # mammals we expect to see. Offset accounts for variation in effort that might have existed
...
# **INVESTIGATE ADDING RANDOM EFFECT, RUNNING AS MAMMAL DETECTION RATES, REMOVING OFFSET TERM, controls for resampling in same sites
...
snakes.sur <- snakes.sur %>%
  mutate(Species = ifelse(Species == "Helicops_angulatus_", 
                          "Helicops_angulatus", Species)) # TEMPORARY FIX FOR HELICOPS ANGULATUS FIX LATER 

snakes.sur <- snakes.sur %>%
  separate(Species, c("Genus", "Species"), sep="_")

temp <- enviro %>%
select(Date, Meters) # made temporary df with just date and effort
...
temp$st.date.meters <- paste(temp$Station, temp$Start_Date, temp$Meters, sep = "//")
...
temp <- enviro %>%
select(Date, Station, Meters) #bring the zeros back

snk.sur.mat2 <- left_join(temp, snk.sur.mat)
snk.sur.mat2 <- snk.sur.mat2[,-4] #remove 'Start Date' column

length(unique((snk.sur.mat2$Date))) #Making sure all survey nights are included

snk.sur.mat2[is.na(snk.sur.mat2)] <- 0 #populate NAs with zeros

snk.sur.mat <- snk.sur.mat2 #back to original dataframe
...
temp <- enviro %>%
select(Date, Station, Meters)

edgelist <- left_join(temp, edgelist)
edgelist <- edgelist[,-4] #remove Start Date column

edgelist$N_per_500 <- (edgelist$n/edgelist$Meters)*500 
...
Fam_snk.sur.mat <- left_join(temp, Fam_snk.sur.mat)
Fam_snk.sur.mat <- Fam_snk.sur.mat[,-4] #remove 'Start Date' column

length(unique((Fam_snk.sur.mat$Date))) #Making sure all survey nights are included

Fam_snk.sur.mat[is.na(Fam_snk.sur.mat)] <- 0 #populate NAs with zeros

edgelist2 <- pivot_longer() #NEED TO CREATE A FAMILY, NAMES2, MAKE FAMILY COLUMN, IT WILL POPULATE EVERY CELL WITH THE DETECTION NUMBERS FROM FAM DATAFRAMER df <- df %>% pivot_longer
```
...
snake.master$Notes[snake.master$Notes == ""] <- NA
...
snake.master$Genus_Species <- paste(snake.master$Genus, snake.master$Species, sep="_")

unique(snake.master$Genus_Species)
...
snake.master$week_num <- strftime(snake.master$Date, format = "%V")
# also need to double check this did what we want. Cursory look seems good.

# now, use dplyr (tidyverse) functions to aggregate the data
matrix.step.one <- snake.master %>%
