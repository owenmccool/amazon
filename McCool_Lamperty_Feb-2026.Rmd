...
# FIND LIT FOR DISTANCE SAMPLING IN TROPICAL FORESTS (OWEN)
...
snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m <= 1, "0_1m", NA))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 0 & Distance_From_Trail_m <=1, "0_1m", Distance_bin_m))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 1 & Distance_From_Trail_m <=2, "1_2m", Distance_bin_m))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 2 & Distance_From_Trail_m <=3, "2_3m", Distance_bin_m))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 3 & Distance_From_Trail_m <=4, "3_4m", Distance_bin_m))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 4 & Distance_From_Trail_m <=5, "4_5m", Distance_bin_m))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 5 & Distance_From_Trail_m <=7, "5_7m", Distance_bin_m))

snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 7 & Distance_From_Trail_m <=11.5, "7_12m", Distance_bin_m))
...
density_est_df[8:14][is.na(density_est_df[8:14])] <- 0
...
# Seasons
# So we technically have a multi season study. Because our data is sparse and it is only two seasons, I don't know if it is the right course of action to use multi season modeling or to just include year as a covariate. This is further complicated by the fact that there was much more effort the second year. Shit. I might suggest we combine the years for now and see what others say. Essentially treat it as a single season with the argument that our goal is comparison, not accurate density estimates per station. We would just need to be sure to present the results as a relative difference and not infer much from the actualy model estimates of density because our data is 2 seasons but without accounting for year, that could inflate our results... I'm not sure. 

# separate out the covariates
covs <- density_est_df[,c("Station", "Diet_Type", "Segment", "Seg_dist_m")]

umf <- unmarkedFrameDS(y=as.matrix(density_est_df[,8:14]), siteCovs=data.frame(covs), dist.breaks=c(0, 1, 2, 3, 4, 5, 7, 12), unitsIn="m", survey="line", tlength=covs$Seg_dist_m)

hist(umf, xlab="distance (m)", main="", cex.lab=0.8, cex.axis=0.8)

# we need to do more reading on figuring out approp distance bins (we = Owen)
```
...
null <- distsamp(~1 ~ 1, umf, keyfun = "halfnorm", output = "density", unitsOut = "kmsq")

summary(null)

# Fit model 2, station only
station <- distsamp(~1 ~ Station, keyfun = "halfnorm", output = "density", umf, unitsOut = "kmsq")

summary(station)

# fit model 3, diet type only
diet_type <- distsamp(~1 ~ Diet_Type, keyfun = "halfnorm", output = "density", umf, unitsOut = "kmsq")

summary(diet_type)

# fit model 4, station + interaction term for station X diet type, station is viewed on its own and as an interaction term w/ diet type
stationXdiet_type <- distsamp(~1 ~ Station/Diet_Type, keyfun = "halfnorm", output = "density", umf, unitsOut = "kmsq")
...
# put mods in a list
mod.set.1 <- list(null = null, hunting = station, guild = diet_type, huntingXguild = stationXdiet_type)
...
aic_table.1
```
Based on this, the top model includes a fixed effect of station (hunting/defaunation status) and the interaction term between hunting status and dietary group (eats mammals vs does not eat mammals).
...
covs.shortened <- density_est_df.shortened[,c("Station", "Diet_Type", "Segment", "Seg_dist_m", "temp_scaled", "hum_scaled")]

umf.shortened <- unmarkedFrameDS(y=as.matrix(density_est_df.shortened[,8:10]), siteCovs=data.frame(covs.shortened), dist.breaks=c(0, 3, 6, 11.5), unitsIn="m", survey="line", tlength=covs.shortened$Seg_dist_m)

# null with shortened df
null.shortened <- distsamp(~1 ~ 1, keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(null.shortened)

# humidity
humidity <- distsamp(~1 ~ hum_scaled,keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(humidity)

# temperature
temperature <- distsamp(~1 ~ temp_scaled, keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(temperature)

# make a table for easy model comparison

# put mods in a list
mod.set.2 <- list(null = null.shortened, humidity = humidity, temperature = temperature)
...
```{r clean density mods}
rm(humidity, null, temperature, null.shortened, station, diet_type, mod.set.1, mod.set.2, umf.shortened, covs.shortened, density_est_df.shortened)
```

Including humidity or temperature does not substantially improve model fit relative to the null (based on AIC values). We should leave it out for obtaining density estimates unless we decide we want per-year estimates, in which case, we should probably incorporate it since those are so different between years.

### Predictions

```{r model fit check}

#When covariates are present, predict in conjunction with the newdata argument should be used. Here, we request the value of sigma when station = YRS

newdata_temporary <- data.frame(Station = "YRS", Diet_Type = "includes_mammals")

predict(stationXdiet_type, type = 'state', newdata=newdata_temporary, appendData = TRUE)

rm(newdata_temporary)

# now lets build a full new data dataframe and check out predictions - my understanding is the values are per square km estimates

newdata <- data.frame(Station=factor(c('YRS', 'YRS', 'TBS', 'TBS')), Diet_Type = factor(c('includes_mammals', 'no_mammals', 'includes_mammals', 'no_mammals')))

density_estimates <- predict(stationXdiet_type, type = 'state', newdata=newdata, appendData = TRUE) #extract density rather than detection probability

ggplot(density_estimates, aes(x=Station, y=Predicted, color = Diet_Type, group = Diet_Type)) + geom_errorbar(aes(ymin =lower, ymax = upper), position=position_dodge(width = 0.9), width = 0.01) + geom_point(position = position_dodge(width=0.9), size = 3) + scale_color_manual(values = c("black", "darkgray")) + theme_bw() + ylab("Individuals/Km2") + ggtitle("Estimated density with 95% CI\nSnakes by guild") + scale_x_discrete(labels=c("Unhunted site", "Hunted site")) + theme(axis.text = element_text(color = "black", size = 12)) 

```
Estimates with standard error

```{r plot SE}
ggplot(density_estimates, aes(x=Station, y=Predicted, color = Diet_Type, group = Diet_Type)) + geom_errorbar(aes(ymin = Predicted - SE, ymax = Predicted + SE), position=position_dodge(width = 0.9), width = 0.25) + geom_point(position = position_dodge(width=0.9), size = 3) + scale_color_manual(values = c("black", "darkgray")) + theme_bw() + ylab("Individuals/Km2") + ggtitle("Estimated density with SE\n Snakes by guild") + scale_x_discrete(labels=c("Unhunted site", "Hunted site")) + theme(axis.text = element_text(color = "black", size = 12)) 

```

Feb 26, 2026

Stopping here for now. At the very least we can say that our qualitative trend actually does support the hypothesis. We need to check if the model is meeting assumptions, if it is okay to combine our seasons in this context, and how to address SE versus 95% confidence intervals in the presentation of the data. Think about which bins to use and figure our multi-season approach?
