...
hist(snakes.sur$Distance_From_Trail_m, breaks = seq(0, 12, by = 0.25))
...
# 2-3
# 3-4
# 4-8.5 (put the 11.5m one in that bin)
...
# 0-0.5
snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m <= 0.5, "0_0.5m", NA))

# 0.5-1
snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 0.5 & Distance_From_Trail_m <= 1, "0.5_1m", Distance_bin_m))

# 1-1.5
snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 1 & Distance_From_Trail_m <= 1.5, "1_1.5m", Distance_bin_m))

# 1.5-2
snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 1.5 & Distance_From_Trail_m <= 2, "1.5_2m", Distance_bin_m))
...
# 4-8.5
snakes.sur <- snakes.sur %>%
  mutate(Distance_bin_m = ifelse(Distance_From_Trail_m > 4, "4_8.5m", Distance_bin_m))
...
density_est_df[9:15][is.na(density_est_df[9:15])] <- 0
...
umf <- unmarkedFrameDS(y=as.matrix(density_est_df[,9:15]), siteCovs=data.frame(covs), dist.breaks=c(0, 0.5, 1, 1.5, 2, 3, 4, 8.5), unitsIn="m", survey="line", tlength=covs$Seg_dist_m)
...
# Fit model 2, incorporate year
year <- distsamp(~1 ~ Year, keyfun = "halfnorm", output = "density", umf, unitsOut = "kmsq")

summary(year)
# huge improvement on model fit
# so keep year moving forward

# fit model 3, station and year
station.year <- distsamp(~1 ~ Year + Station, keyfun = "halfnorm", output = "density", umf, unitsOut = "kmsq")

summary(station.year)
# station on its own doesnt appear to have an overall effect on snake density (not accounting for differences between diet types)

# fit model 5, year, station + interaction term for station X diet type, station is viewed on its own and as an interaction term w/ diet type
stationXdiet_type <- distsamp(~1 ~ Year + Station/Diet_Type, keyfun = "halfnorm", output = "density", umf, unitsOut = "kmsq")

summary(stationXdiet_type)
# huge improvement in model fit

# Make a table to compare models

# put mods in a list
mod.set.1 <- list(null = null, year.only = year, hunting.year = station.year, huntingXguild.year = stationXdiet_type)
...
umf.shortened <- unmarkedFrameDS(y=as.matrix(density_est_df.shortened[,9:15]), siteCovs=data.frame(covs.shortened), dist.breaks=c(0, 0.5, 1, 1.5, 2, 3, 4, 8.5), unitsIn="m", survey="line", tlength=covs.shortened$Seg_dist_m)

# null with shortened df
null.shortened <- distsamp(~1 ~ Year, keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(null.shortened)

# humidity
humidity <- distsamp(~1 ~ Year + hum_scaled,keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(humidity)

# temperature
temperature <- distsamp(~1 ~ Year + temp_scaled, keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(temperature)

temperature.station <- distsamp(~1 ~ Year + temp_scaled + Station, keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(temperature.station)

temperature.stationXguild <- distsamp(~1 ~ Year + temp_scaled + Station/Diet_Type, keyfun = "halfnorm", output = "density", umf.shortened, unitsOut = "kmsq")

summary(temperature.stationXguild)

# put mods in a list
mod.set.2 <- list(null = null.shortened, humidity = humidity, temperature = temperature, temp.station = temperature.station, temp.stationXguild = temperature.stationXguild)
...
I examined Year, and it may improve model fit but I still am unsure if for our purposes it makes sense to use it. Temperature with Year being included appears to matter after a preliminary check. Need to carefully go through this. I also regrouped into smaller bins, which seems better but we should probably group 1-2 together because the curve still doesn't fit great. 
