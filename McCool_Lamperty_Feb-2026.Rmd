...
```{r clean after temp}
rm(sim, temp.mean.se, temp.mod, temperature.hist)
```
...
snakes.sur %>%
  group_by(Year, Station) %>%
    reframe(N_surveys = length(unique(Start_Date))) #Effort comparisons between stations (WHERE DID 'EFFORT' GO FROM snakes.sur)

enviro %>%
  group_by(Season, Station) %>%
    reframe(Accum_hrs = sum(Effort_hrs))

# FIND LIT FOR DISTANCE SAMPLING IN TROPICAL FORESTS (OWEN -see my note below)
...
# Mod 2
year.det <- distsamp(~ Year ~ 1, umf, unitsOut = "kmsq")

summary(year.det)

# Mod 3

year.det.den <- distsamp(~ Year ~ Year, umf, unitsOut = "kmsq")

summary(year.det.den)

# Mod 4
station <- distsamp(~Year ~ Year + Station, umf, unitsOut = "kmsq")

summary(station)

# Mod 5
stationXdiet_type <- distsamp(~Year ~ Year + Station/Diet_Type, umf, unitsOut = "kmsq")

summary(stationXdiet_type)

# Make a table to compare models

# put mods in a list
mod.set.1 <- list(null = null, year.det = year.det, year.det.den = year.det.den, huntingXguild.year = stationXdiet_type)
...
Just to be sure, let's look at how humidity and temperature affect model AIC values. 
...
# null with shortened df
null2 <- distsamp(~1 ~ 1, umf.shortened, unitsOut = "kmsq")

# mod 1
yr.det2 <- distsamp(~Year ~ 1, umf.shortened, unitsOut = "kmsq")

# mod 2
yr.det.den2 <- distsamp(~Year ~ Year, umf.shortened, unitsOut = "kmsq")

# mod 3 
yr.temp <- distsamp(~Year ~ Year + temp_scaled, umf.shortened, unitsOut = "kmsq")

yr.station <- distsamp(~Year ~ Year + Station, umf.shortened, unitsOut = "kmsq")

yr.temp.station <- distsamp(~Year ~ Year + temp_scaled + Station, umf.shortened, unitsOut = "kmsq")

yr.temp.station.guild <- distsamp(~Year ~ Year + temp_scaled + Diet_Type*Station, umf.shortened, unitsOut = "kmsq")

yr.temp.hum.station.guild <- distsamp(~Year ~ Year + temp_scaled + Diet_Type*Station + hum_scaled, umf.shortened, unitsOut = "kmsq")

# put mods in a list
mod.set.2 <- list(null = null2, yr.det = yr.det2, yr.det.den = yr.det.den2, yr.station = yr.station, yr.temp.station = yr.temp.station, yr.temp.station.guild = yr.temp.station.guild, yr.temp.hum.station.guild = yr.temp.hum.station.guild)
...
# so the top model includes temperature, station, year, and an interaction between station and dietary guild. This needs to be rechecked.

summary(yr.temp.station.guild)

```
So the top model includes temperature, station, year, and an interaction between station and dietary guild.
