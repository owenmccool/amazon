...
```{r parent trails and segments}
# to understand survey makeup per night, look at segments for each parent trail long name

trails <- snakes.sur %>%
  group_by(Parent_Trail_LongName) %>%
  reframe(unique(Segment))

enviro %>%
  reframe(unique(Parent_Trail_LongName))

# also it doesn't seem that the enviro Parent Trail Long Names match those in enviro - but as of 2/23 T has not put much thought or effort into verifying that. 
```
...
covs <- density_est_df[,c("Station", "Diet_Type", "Segment", "Seg_dist_m", "Temperature_F", "Humidity")]
