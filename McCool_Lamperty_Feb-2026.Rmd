...
# NOTE FOR OWEN: repeat what I did with the effort time with the distance
```
...
temp.mod <- glmmTMB(Temperature ~ Station + Season + (1|Parent_Trail_LongName), data = temperature_long, family = gaussian()) #is this the right family argument?

sim <- simulateResiduals(fittedModel = temp.mod, plot = T)

summary(temp.mod) # HOW TO INTERPRET OUTPUT? CAN'T FIND REFERENCE GROUPS FOR 
...
hum.mod <- glmmTMB(Humidity ~ Station + Season + (1|Parent_Trail_LongName), data = humidity_long, family = gaussian()) #is this the right family argument?
...
zeros <- snakes.sur %>%
  ...
  select(Start_Date, Station, Diet_Type, Segment, Seg_dist_m)
...
survey.tally <- surveys.no.zeros %>%
  group_by(Start_Date, Station, Diet_Type, Segment, Seg_dist_m, Distance_bin_m) %>% tally()
...
# make NAs zeros
density_est_df[is.na(density_est_df)] <- 0
```
...
umf <- unmarkedFrameDS(y=as.matrix(density_est_df[,6:8]), siteCovs=data.frame(covs), dist.breaks=c(0, 3, 6, 11.5), unitsIn="m", survey="line", tlength=covs$Seg_dist_m)

# NEED TO VERIFY 1 BIN DETERMINATION RULES OF THUMB
# ALSO, DO WE NEED TO MODIFY TLENGTH BY 1000?
