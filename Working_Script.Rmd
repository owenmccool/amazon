...
snake.sur$Date_Start_Time <- ymd_hm(snake.sur$Date_Start_Time) #ymd_hm is year, month, day & hours, minutes
...
# what class is the Date_End_Time column?, says its a character
class(snake.sur$Date_End_Time)

# currently a 'character', we want a date-time object, fix this:
snake.sur$Date_End_Time <- ymd_hm(snake.sur$Date_End_Time)
...
snake.sur$Date_End_Time[which(snake.sur$Survey_End_Time == "12:05 AM" | snake.sur$Survey_End_Time == "12:11 AM")]<-  snake.sur$Date_End_Time[which(snake.sur$Survey_End_Time == "12:05 AM" | snake.sur$Survey_End_Time == "12:11 AM")] + lubridate::ddays(x =1) # i don't understand this part still after the + lubridate()
...
tbs.no.mammals <- snake.master[snake.master$Station == "TBS" & 
                                 snake.master$Diet_Type == "no mammals", ] #getting counts for all TBS guilds

tbs.mixed <- snake.master[snake.master$Station == "TBS" & 
                                 snake.master$Diet_Type == "mixed", ]

tbs.mammals <- snake.master[snake.master$Station == "TBS" & 
                                 snake.master$Diet_Type == "mostly mammals", ]


yrs.no.mammals <- snake.master[snake.master$Station == "YRS" & 
                                 snake.master$Diet_Type == "no mammals", ] #getting counts for all YRS guilds

yrs.mixed <- snake.master[snake.master$Station == "YRS" & 
                                 snake.master$Diet_Type == "mixed", ]

yrs.mammals <- snake.master[snake.master$Station == "YRS" & 
                                 snake.master$Diet_Type == "mostly mammals", ]
yrs.unknown <- snake.master[snake.master$Station == "YRS" & 
                                 snake.master$Diet_Type == "unknown", ] #getting counts for all TBS guilds

mod1 <- glm(snake.master$Family ~ snake.master$Station, family = "poisson")
```
...
surveys.master <- snake.sur %>%
  distinct(Start_Date, Date_Start_Time, Date_End_Time, Effort_Hrs, Station, Zone, Surveyors, Weather) #look across these columns, and where all the values across a row are the same( eg. date, station, time, etc.) to avoid duplicates: one copy of each survey
...
temp <- snake.sur %>%
  group_by(Start_Date, Station, Zone, Common_Name, Effort_Hrs) %>% tally()


# make an extra column that has the station ID and trail walked and date, separated by //

temp$st.date.zone <- paste(temp$Station, temp$Start_Date, temp$Zone, temp$Effort_Hrs, sep = "//")
